...
class EvalCallbackWithMinSteps(EvalCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if not continue_training:
            return False
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            current = self.last_mean_reward
            if current is not None:
                if self._best_mean_reward is None or current >= self._best_mean_reward - self._improvement_eps:
                    if self._best_mean_reward is None or current > self._best_mean_reward:
                        self._best_mean_reward = current
                    self._no_improvement_evals = 0
                else:
                    self._no_improvement_evals += 1
                    if self._no_improvement_evals >= self.patience:
                        if self.verbose > 0:
                            logging.getLogger(__name__).info("Early stopping: no improvement on eval.")
                        return False
        return True
...
def train(algo_name: str, env_name: str, total_timesteps: int = None, total_episodes: int = None, hyperparams: dict = None):
    ...
    try :
        ...
        if algo_name == "a2c":
            ...
            kwargs.setdefault("n_steps", 256)
            kwargs.setdefault("batch_size", 1024)

        # Override with hyperparams if provided
        if hyperparams:
            logger.info("Overriding defaults with: %s", hyperparams)
            ...
            if algo_name in ["ppo", "a2c"]:
                ...
                if "clip_range" in hyperparams and algo_name == "ppo": kwargs["clip_range"] = hyperparams["clip_range"]
        ...
        optimizer_kwargs = {}
        if hyperparams and "weight_decay" in hyperparams:
            optimizer_kwargs["weight_decay"] = hyperparams["weight_decay"]
        ...
        # PPO batch_size should not exceed rollout size (n_steps * n_envs)
        if algo_name == "ppo":
            rollout_steps = agent_kwargs.get("n_steps", kwargs.get("n_steps", 256)) * n_envs
            batch_size = agent_kwargs.get("batch_size")
            if batch_size is not None and batch_size > rollout_steps:
                logger.warning("Reducing batch_size from %s to %s to fit rollout size.", batch_size, rollout_steps)
                agent_kwargs["batch_size"] = rollout_steps
        ...
        if total_episodes is not None:
            logger.info("Training for %s episodes...", total_episodes)
        ...
        if not os.path.exists(best_model_path):
            logger.warning("Best model not found. Saving final model as best model.")
        ...
        # Odłącz env od agenta (zamyka TB writers itp.)
        try:
        ...
        for e in (env, eval_env, holdout_eval_env):
            try:
                if e is not None:
                    e.close()
            except Exception:
